...
function observablesPromise() {
  const observableRequestOptions = {
    url: 'https://beta.observablehq.com/@chrisamurao',
    headers: {
      'User-Agent': 'christopher-amurao-personal-website',
    },
  }
...
function gistsPromise() {
  const gistsRequestOptions = {
    url: 'https://api.github.com/users/chrisamurao/gists?per_page=100',
    headers: {
      'User-Agent': 'my-blog',
    },
    json: true,
...
function gistsListFromPayload(gistsListPayload) {
  return gistsListPayload
    ...
    .map(gist => ({
      ...
      href: `https://bl.ocks.org/chrisamurao/${gist.id}`,
...
function observablesListFromObservableSiteHTML(siteHTML) {
  ...
  return preloadData.notebooks
    ...
    .map(notebook => ({
      alt: notebook.title,
      createdAt: null,
      href: `https://beta.observablehq.com/@chrisamurao/${notebook.slug}`,
